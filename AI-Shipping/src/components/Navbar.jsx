    ...
    <nav className="fixed top-4 left-0 right-0 z-50 px-4 md:px-8">
      <div className="w-full max-w-[1280px] mx-auto flex items-center justify-between px-6 md:px-8 py-4 bg-white/80 backdrop-blur-md rounded-full shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <Link to="/contact" className="hidden sm:flex bg-[#b61722] text-white hover:bg-[#930013] duration-200 transition-all font-semibold px-4 py-1.5 rounded-lg text-xs cursor-pointer items-center gap-1.5">
            Get Connected
      ...
      {menuOpen && (
        <div className="md:hidden mt-2 mx-0 bg-white/95 backdrop-blur-lg rounded-3xl p-6 shadow-lg flex flex-col gap-3 border border-[#E2E8F0]/20 animate-mobile-menu">
          ...
          <Link to="/contact" onClick={() => setMenuOpen(false)} className="sm:hidden mt-2 bg-[#b61722] text-white hover:bg-[#930013] duration-200 transition-all font-semibold px-4 py-2 rounded-lg text-xs cursor-pointer flex items-center justify-center gap-1.5">
            Get Connected
            <span className="material-symbols-outlined text-[16px]">arrow_outward</span>
          </Link>
        </div>
